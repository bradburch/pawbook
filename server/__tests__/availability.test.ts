...
describe('availability API — regression guards', () => {
  ...
  it('walks are rejected only on blocked days', async () => {
    ...
    const onBlocked = (await (
      await app.request('/api/brad-paws/availability?type=walk&start=2028-07-03', {}, env)
    ).json()) as { available: boolean };
    const onBusy = (await (
      await app.request('/api/brad-paws/availability?type=walk&start=2028-06-21', {}, env)
